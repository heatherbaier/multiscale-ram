...
class miniConv(torch.nn.Module):
    ...
    def __init__(self, resnet):
        ...
        self.fc1 = torch.nn.Linear(2 * 128, 128)#.to('cuda:1')
...
checkpoint = torch.load("./ckpt/ram_8_50x50_0.75_model_best.pth.tar")
...
for im_name, patches in glimpses.items():
    ...
    for glimpseID, coords in patches.items():  
        ...
        next_glimpse_size = int(size * .75)
        from_x, to_x, from_y, to_y = from_x, from_x + next_glimpse_size, from_y, from_y + next_glimpse_size
                
        cur_patch = image[:, :, from_x:to_x, from_y:to_y]
        cur_patch = torch.nn.functional.interpolate(cur_patch, size = (size, size), mode = 'nearest')        
        patches.append(cur_patch)
