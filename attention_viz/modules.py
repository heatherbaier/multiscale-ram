...
#         input_size = 64
    ...
    def forward(self, h_t, last, muni_id, epoch):
        ...
        if last:
            ...
            path = "epoch" + str(epoch) + "_dists.json"
                        
            key = muni_id + "_" + str(epoch)
            ...
            if os.path.exists(path):
                ...
                cur[key] = get_dists(mu, self.std, 50)
                ...
                with open(path, "w") as f:
                    json.dump({key: get_dists(mu, self.std, 50)}, f)               
        
        
#         print("MU: ", mu)
#         print("MU shape: ", mu.shape)
        
#         print("DIST: ", torch.distributions.Normal(mu, self.std))
# #         print("DIST shape: ", torch.distributions.Normal(mu, self.std).shape)        
        
#         print("DIST SAMPLE shape: ", torch.distributions.Normal(mu, self.std).rsample())        
#         print("DIST SAMPLE shape: ", torch.distributions.Normal(mu, self.std).rsample().shape)        
