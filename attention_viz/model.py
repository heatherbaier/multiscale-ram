...
class RecurrentAttention(nn.Module):
    ...
    def forward(self, x, l_t_prev, h_t_prev, muni_id, epoch, last = False):
        ...
        log_pi, l_t = self.locator(h_t, last, muni_id, epoch)
        b_t = self.baseliner(h_t).squeeze()

        if last:
            cont_pred = self.classifier(h_t)
            return h_t, l_t, b_t, log_pi, cont_pred
